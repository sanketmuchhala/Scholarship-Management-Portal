...
exports.postIssueBook = async(req, res, next) => {
    if(req.user.violationFlag) {
        req.flash("error", "You are flagged for violating rules. We're setting up an inquiry to test you violations on policy. Untill the flag is lifted, You can't issue any scholarships");
        return res.redirect("back");
    }

    if(req.user.bookIssueInfo.length >= 5) {
        req.flash("warning", "You can't issue more than 5 Scholarships at a time");
        return res.redirect("back");
    }
